...
function TrackVehicleByPlate(plate)
    QBCore.Functions.TriggerCallback('Batz-Garages:server:GetVehicleLocation', function(coords)
...
local function PublicGarage(garageName, type)
    ...
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "Batz-Garages:client:GarageMenu",
                args = {
                    garageId = garageName,
                    garage = garage,
...
local function ExitAndDeleteVehicle(vehicle)
    ...
    if Config.SpawnVehiclesServerside then
        Wait(1000)
        TriggerServerEvent('Batz-Garages:server:parkVehicle', plate)
    end
end
...
local function ParkOwnedVehicle(veh, garageName, vehLocation, plate)
    ...
    if plate then
        ...
        TriggerServerEvent('Batz-Garages:server:UpdateOutsideVehicles', OutsideVehicles)
    end
...
local function AddRadialParkingOption()
    ...
    if (veh and dist <= Config.VehicleParkDistance and Config.AllowParkingFromOutsideVehicle) or IsPedInAnyVehicle(ped) then
        MenuItemId1 = exports['qb-radialmenu']:AddOption({
            ...
            icon = 'square-parking',
            type = 'client',
            event = 'Batz-Garages:client:ParkVehicle',
            shouldClose = true
        }, MenuItemId1)
    end
    MenuItemId2 = exports['qb-radialmenu']:AddOption({
        ...
        icon = 'warehouse',
        type = 'client',
        event = 'Batz-Garages:client:OpenMenu',
        shouldClose = true
    }, MenuItemId2)
end
...
function JobMenuGarage(garageName)
    ...
    for veh, label in pairs(vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "Batz-Garages:client:TakeOutGarage",
                args = {
                    vehicleModel = veh,
                    garage = garage
                }
...
RegisterNetEvent("Batz-Garages:client:GarageMenu", function(data)
            ...
            for k, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions+1] = {
                        ...
                        params = {
                            event = "Batz-Garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
...
RegisterNetEvent('Batz-Garages:client:TakeOutGarage', function(data, cb)
...
RegisterNetEvent('Batz-Garages:client:OpenMenu', function()
    ...
    elseif CurrentHouseGarage then
        TriggerEvent('Batz-Garages:client:OpenHouseGarage')
    end
end)

RegisterNetEvent('Batz-Garages:client:ParkVehicle', function()
    local ped = PlayerPedId()
    local canPark = true
...
RegisterNetEvent('Batz-Garages:client:ParkLastVehicle', function(parkingName)
    local ped = PlayerPedId()
...
RegisterNetEvent('Batz-Garages:client:TakeOutDepot', function(data)
    local vehicle = data.vehicle
    ...
    if not vehExists then
        ...
        if PlayerData.money['cash'] >= vehicle.depotprice or PlayerData.money['bank'] >= vehicle.depotprice then
            TriggerEvent("Batz-Garages:client:TakeOutGarage", data, function (veh)
                if veh then
...
RegisterNetEvent('Batz-Garages:client:TrackVehicleByPlate', function(plate)
    TrackVehicleByPlate(plate)
end)

RegisterNetEvent('Batz-Garages:client:OpenHouseGarage', function()
    MenuHouseGarage()
end)

RegisterNetEvent('Batz-Garages:client:setHouseGarage', function(house, hasKey)
    if hasKey then
...
RegisterNetEvent('Batz-Garages:client:houseGarageConfig', function(garageConfig)
...
RegisterNetEvent('Batz-Garages:client:addHouseGarage', function(house, garageInfo)
